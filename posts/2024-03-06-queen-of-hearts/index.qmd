...
Given that your ticket is selected, you have probability $\Pr(\text{Number matches}) = 1/k$ that your number matches. Therefore, the probability that you win is \[ \Pr(\text{You
win})= \Pr(\text{Your ticket selected} \cap \text{Number matches}) = \frac{n}{k(N + n)}.\
\]
...
The game ends exactly when somebody wins, and we saw earlier that when there are $k$ tickets left, the probability that somebody wins is $1/k$. Let $L$ denote the length of the game. Then $L$ can be any number $1,2,\ldots,54$. But $\Pr(L = 1)$ is \[ \Pr(L = 1) = \Pr( \text{Somebody wins
...
Suppose you only started playing the game this week. Denote your winnings by $W$, where we understand that if you lose then your winnings are negative. You spent $\$n$ on tickets, but you will have $\$(J + N + n)$ if you win. Your expected winnings are \[ \mathbb{E}(W) = -n \Pr(\text{Lose}) + (J + N + n)\Pr(\text{Win}), \] and after plugging in the formulas from above we get \[ \mathbb{E}(W) = -n \left( 1 - \frac{n}{k(N + n)}\right) + (J + N + n)\left( \frac{n}{k(N + n)} \right). \]
