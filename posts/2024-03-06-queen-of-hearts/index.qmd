...
date: "2024-03-06"
categories: [news, probability, classical]
image: preview.jpg
twitter-card: 
  image: preview.jpg
draft: false
...
\[ 
\Pr(X = x,\vert,X\neq x_{1},\ldots,x_{m}) = \frac{\Pr(X = x)}{\Pr(X \neq x_{1},\ldots,x_{m})} = \frac{1/54}{(54 - m)/54} = \frac{1}{k},\quad x = 1,2,\ldots,54,\quad x \neq x_{1},\ldots,x_{m}.\
\]
...
The game ends exactly when somebody wins, and we saw earlier that when there are $k$ tickets left, the probability that somebody wins is $1/k$. Let $L$ denote the length of the game. Then $L$ can be any number $1,2,\ldots,54$. But $\Pr(L = 1)$ is 
\[ 
...
Suppose you only started playing the game this week. Denote your winnings by $W$, where we understand that if you lose then your winnings are negative. You spent $\$n$ on tickets, but you will have $\$(J + N + n)$ if you win. Your expected winnings are 
\[ 
